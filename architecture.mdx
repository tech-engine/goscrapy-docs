---
title: "GoScrapy architecture and request lifecycle"
description: "How requests flow through the GoScrapy engine, scheduler, worker pool, middlewares, and pipelines in an event-driven, concurrent Go architecture."
---

GoScrapy's data flow is designed for clarity and concurrent execution, utilizing a robust, event-driven engine built natively for Go's concurrency model.
...
5. **HTTP Adapter**: The network layer that performs the actual data fetching.
6. **Middlewares**: Pluggable hooks for modifying requests/responses.
