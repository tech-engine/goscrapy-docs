...
- **`TelemetryHub`**: The central brain. It wakes up at a fixed interval (default 500ms), asks all collectors for a snapshot, and broadcasts a `GlobalSnapshot` to all observers.
- **`IStatsCollector`**: Components that gather metrics silently in the background (like counting HTTP requests or scraped items).
- **`IStatsObserver`**: Components that listen to the `TelemetryHub` and display or export the metrics (like a dashboard, JSON logger, or Prometheus).
- **`GlobalSnapshot`**: A read-only snapshot containing the current timestamp, uptime, and the data from all collectors.
